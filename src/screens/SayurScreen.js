...
export default function SayurScreen() {
  ...
  const filterProducts = products
    .filter((item) => {
      ...
      return category.includes("sayur") && (search === "" || name.includes(search.toLowerCase()));
    })
    .map(item => ({
      id: item.id,
      ...
      <View style={{ flex: 1 }}>
        ...
        <View style={categoryStyles.headerSection}>
          <Text style={categoryStyles.categoryTitle}>
            Kategori <Text style={categoryStyles.boldGreen}>Sayuran</Text>
          </Text>
          <Text style={categoryStyles.headerSubtitle}>Sayuran segar langsung dari petani pilihan</Text>
        </View>
        ...
        {loading ? (
          ...
          <View style={categoryStyles.emptyContainer}>
            ...
            <Text style={categoryStyles.emptyText}>Sayuran tidak tersedia saat ini</Text>
          </View>
        )}
      </View>
